...
namespace agora
{
    namespace common
    {
        ...
        VideoDeviceManager::VideoDeviceManager(rtc::IRtcEngine *mRtcEngine, ERROR_CODE &ret)
        {
        ...
        int
        VideoDeviceManager::callApi(API_TYPE_DEVICE_MANAGER apiType, const std::string& parameters, void*& ptr)
        {
        ...
        int
        VideoDeviceManager::callApi(API_TYPE_DEVICE_MANAGER apiType, const std::string& parameters, void*& ptr, void*&ptr2)
        {
            int ret = ERROR_CODE::ERROR_OK;
            Document document;
            document.Parse(parameters.c_str());
            ...
            switch (apiType)
            {
                case API_TYPE_DEVICE_MANAGER::GET_CURRENT_DEVICE_INFO:
                {
                    ...
                    CHECK_RET_ERROR(ret)
                    ret = getCurrentDevice(index, reinterpret_cast<char *>(ptr), reinterpret_cast<char *>(ptr2));
                }
                    break;
        ...
        int VideoDeviceManager::setDevice(const char deviceId[rtc::MAX_DEVICE_ID_LENGTH])
        {
            if (videoDeviceManager && videoDeviceManager->get())
            {
        ...
        int VideoDeviceManager::getCurrentDevice(int index, char deviceName[rtc::MAX_DEVICE_ID_LENGTH], char deviceId[rtc::MAX_DEVICE_ID_LENGTH])
        {
            if (videoDeviceManager && videoDeviceManager->get())
            {
        ...
        void VideoDeviceManager::release()
        {
            delete this;
        }
    }
