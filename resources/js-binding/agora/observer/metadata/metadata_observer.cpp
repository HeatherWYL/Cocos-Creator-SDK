...
namespace agora
{
    namespace common
    {
        ...
        MetadataObserver::MetadataObserver(rtc::IMetadataObserver* _observer):_metadataObserver(_observer)
        {
        ...
        {
            return MAX_METADATA_SIXE;
        }

        bool
        MetadataObserver::onReadyToSendMetadata(Metadata &metadata)
        {
            std::lock_guard<std::mutex> lock(queueMutex);
        ...
        void
        MetadataObserver::onMetadataReceived(const Metadata &metadata)
        {
            std::lock_guard<std::mutex> lock(queueMutex);
        ...
        int
        MetadataObserver::sendMetadata(Metadata *_metadata)
        {
            std::lock_guard<std::mutex> lock(queueMutex);
        ...
        int
        MetadataObserver::setMaxMetadataSize(int size)
        {
