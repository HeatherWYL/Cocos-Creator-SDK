...
namespace agora
{
    namespace common
    {
        ...
        template<typename T>
        void json_to_VideoDimension(T &object, rtc::VideoDimensions &videoDimensions, int &ret)
        {
        ...
        template<typename T>
        void json_to_CameraCapturerConfiguration(T &object, rtc::CameraCapturerConfiguration &cameraCapturerConfiguration, int &ret)
        {
            int preference;
            ...
            int cameraDirection;
            get_parameter_int(object, "cameraDirection", cameraDirection,
                              ret);
            CHECK_RET_ERROR_NULL(ret)
            cameraCapturerConfiguration.cameraDirection = rtc::CAMERA_DIRECTION(cameraDirection);
#endif

            cameraCapturerConfiguration.preference = rtc::CAPTURER_OUTPUT_PREFERENCE(
                    preference);
        }

        template<typename T>
        void json_to_LastmileProbeConfig(T &object, rtc::LastmileProbeConfig &lastmileProbeConfig, int &ret)
        {

            get_parameter_bool(object, "probeUplink", lastmileProbeConfig.probeUplink, ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_bool(object,
                               "probeDownlink",
                               lastmileProbeConfig.probeDownlink,
                               ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_uint(object,
                               "expectedUplinkBitrate",
                               lastmileProbeConfig.expectedUplinkBitrate,
                               ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_uint(object,
                               "expectedDownlinkBitrate",
                               lastmileProbeConfig.expectedDownlinkBitrate,
                               ret);
            CHECK_RET_ERROR_NULL(ret)
        }

        template<typename T>
        void json_to_TranscodingUser(T &object, rtc::TranscodingUser* transcodingUser, const int &count, int &ret)
        {
        ...
        template<typename T>
        void json_to_RtcImage(T &object, rtc::RtcImage& rtcImage, std::string &url, int &ret)
        {
        ...
        template<typename T>
        void json_to_BeautyOptions(T &object, rtc::BeautyOptions* beautyOptions, int &ret)
        {
            ...
            get_parameter_float(object,
                                "lighteningLevel",
                                beautyOptions->lighteningLevel,
                                ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_float(object,
                                "smoothnessLevel",
                                beautyOptions->smoothnessLevel,
                                ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_float(object,
                                "rednessLevel",
                                beautyOptions->rednessLevel,
                                ret);
            CHECK_RET_ERROR_NULL(ret)
        }

        template<typename T>
        void json_to_InjectStreamConfig(T &object, rtc::InjectStreamConfig* injectStreamConfig, int &ret)
        {
            ...
            get_parameter_int(object,
                              "videoFramerate",
                              injectStreamConfig->videoFramerate,
                              ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_int(object,
                              "videoBitrate",
                              injectStreamConfig->videoBitrate,
                              ret);
            CHECK_RET_ERROR_NULL(ret)
            ...
            get_parameter_int(object,
                              ...
                              ret);
            CHECK_RET_ERROR_NULL(ret)
            injectStreamConfig->audioSampleRate = rtc::AUDIO_SAMPLE_RATE_TYPE(audioSampleRate);

            get_parameter_int(object,
                              "audioBitrate",
                              injectStreamConfig->audioBitrate,
                              ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_int(object,
                              "audioChannels",
                              injectStreamConfig->audioChannels,
                              ret);
            CHECK_RET_ERROR_NULL(ret)
        }

        template<typename T>
        void json_to_ChannelMediaInfo(T &object, rtc::ChannelMediaInfo* channelMediaInfo, std::string *channelName, std::string *token, const int &count, int &ret)
        {

            if (count == -1)
            {
                if (object.HasMember("channelName") && object.HasMember("token") && object.HasMember("uid"))
                {
            ...
            {
                rapidjson::Value& value = object[i];
                if (value.HasMember("channelName") && value.HasMember("token") && value.HasMember("uid"))
                {
        ...
        template<typename T>
        void json_to_ChannelMediaOptions(T &object, rtc::ChannelMediaOptions& mediaOptions, int &ret)
        {
            get_parameter_bool(object, "autoSubscribeAudio", mediaOptions.autoSubscribeAudio, ret);
            CHECK_RET_ERROR_NULL(ret);

            get_parameter_bool(object, "autoSubscribeVideo", mediaOptions.autoSubscribeVideo, ret);
            CHECK_RET_ERROR_NULL(ret);
        }

        template<typename T>
        void json_to_ScreenCaptureParameters(T &object, rtc::ScreenCaptureParameters& mediaOptions, rtc::VideoDimensions& videoDimensions, int* windowList, int &ret)
        {
            ...
            get_parameter_bool(object, "captureMouseCursor", mediaOptions.captureMouseCursor, ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_bool(object, "windowFocus", mediaOptions.windowFocus, ret);
            CHECK_RET_ERROR_NULL(ret)

            get_parameter_int(object, "excludeWindowCount", mediaOptions.excludeWindowCount, ret);
            CHECK_RET_ERROR_NULL(ret)
